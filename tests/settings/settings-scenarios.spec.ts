...
test("T22: should verify that Run information popup is displayed correctly.", async ({
  ...
  await expect(runInformationModal.name).toContainText(AttributesRunData.NAME);
  await expect(runInformationModal.id).toContainText(AttributesRunData.ID);
  await expect(runInformationModal.state).toContainText(AttributesRunData.STATE);
  await expect(runInformationModal.owner).toContainText(AttributesRunData.OWNER);
  await expect(runInformationModal.created).toContainText(AttributesRunData.CREATED);
  await expect(runInformationModal.modified).toContainText(AttributesRunData.MODIFIED);
  await expect(runInformationModal.description).toContainText(AttributesRunData.DESCRIPTION);
  await expect(runInformationModal.tags).toContainText(AttributesRunData.TAGS);
  await expect(runInformationModal.groupTags).toContainText(AttributesRunData.GROUP_TAGS);
});
